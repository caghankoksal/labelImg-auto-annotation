...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None,
                ...
                config = "configs/swin_backbone_pretrained_coco.py",
                checkpoint = "models/swin_backbone_pretrained_coco-fulldata_labeltool_epoch_20.pth",
                model_class_file= "data/predefined_model_classes.txt"):
        ...
        # Load predefined classes for the auto-annotation-model
        self.load_model_predefined_classes(predef_model_classes_file=model_class_file)
        ...
        # Threshold for box prediction
        self.score_thr = 0.7    
        # MM detection config file
        self.config = config
        # Checkpoint of the model
        self.checkpoint = checkpoint
        #Auto Label Button
        self.autolabel_button = QPushButton("Auto Label")
        self.autolabel_button.clicked.connect(self.click_autolabel_button)
        # Autoannotation Model
        self.annotation_model = self.create_autolabel_model()
        ...
        list_layout.addWidget(self.autolabel_button)
    ...
    def create_autolabel_model(self):
    ...
    # Auto Label Function
    def click_autolabel_button(self):

        #classes = {0:"person", 1:"bird", 2:"car", 3:"cat", 4:"dog", 5:"person" }
        #classes = { i:v for i,v in enumerate(['person', 'animal', 'onomatopoeia', 'gun', 'knife', 'building','vehicle', 'bicycle'])}
        classes = { i:v for i,v in enumerate(self.model_label_hist)}
        
        result = inference_detector(self.annotation_model, self.file_path)
        # take model 
        ...
        for i,each in enumerate(bboxes):
            xmin, ymin, xmax, ymax, threshold = each
            xmin, ymin, xmax, ymax = int(xmin), int(ymin), int(xmax), int(ymax)
            shapes.append((classes[labels[i]], [(xmin,ymin), (xmax,ymin), (xmax,ymax), (xmin,ymax)], None, None, False))
        ...
        canvas_points = [each.points for each in self.canvas.shapes]
        
        unique_s = []
        if (len(s) > 0):
            unique_s = [ each for each in s if each.points not in canvas_points]

        for each in unique_s:
            self.add_label(each)
        self.update_combo_box()
        
        updated_list = list(set(unique_s + self.canvas.shapes))
        self.canvas.load_shapes(updated_list)
    ...
    def load_model_predefined_classes(self, predef_model_classes_file):
        if os.path.exists(predef_model_classes_file) is True:
            with codecs.open(predef_model_classes_file, 'r', 'utf8') as f:
                for line in f:
                    line = line.strip()
                    if self.model_label_hist is None:
                        self.model_label_hist = [line]
                    else:
                        self.model_label_hist.append(line)
...
def get_main_app(argv=[]):
    ...
    # Model classes
    argparser.add_argument("--model_class_file",
                           default=os.path.join(os.path.dirname(__file__), "data", "model_predefined_classes.txt"),
                           nargs="?")


    #Auto Annotation MMDet configs and checkpoint
    argparser.add_argument("--config", default="configs/swin_backbone_pretrained_coco.py")
    argparser.add_argument("--checkpoint", default="models/swin_backbone_pretrained_coco-fulldata_labeltool_epoch_20.pth")
